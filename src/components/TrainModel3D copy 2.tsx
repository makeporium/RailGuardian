...
export const TrainModel3D: React.FC<TrainModel3DProps> = ({ trainData }) => {
  ...
  return (
    ...
    >
      ...
      {/* Train */}
      <div className="flex flex-wrap justify-center gap-3 max-w-5xl overflow-hidden">
        {/* Engine */}
        <div className="w-16 h-12 bg-gradient-to-r from-blue-600 to-blue-700 rounded-l-xl rounded-r-md shadow-lg flex items-center justify-center text-white text-xs font-bold animate-slide-in-left">
          ENG
        </div>
        ...
        {trainData.coaches.map((coach, index) => (
          ...
          >
            ...
            {hoveredCoach?.id === coach.id && (
              <div className="absolute bottom-16 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white text-xs rounded-lg p-3 z-20 whitespace-nowrap animate-fade-in-up">
